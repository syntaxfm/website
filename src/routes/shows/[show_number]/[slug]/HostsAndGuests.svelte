<script lang="ts">
	import type { Guest } from '@prisma/client';

	export let guests: Guest[];
</script>

<div class="guests-and-hosts">
	<h5>Hosted By:</h5>
	<div class="featuring">
		<figure>
			<img src={`https://github.com/wesbos.png`} alt="" />
			<figcaption>
				<p>Wes Bos</p>
			</figcaption>
		</figure>
		<figure>
			<img src={`https://github.com/stolinski.png`} alt="" />
			<figcaption>
				<p>Scott Tolinski</p>
				<div class="featuring_socials">
					<svg
						xmlns="http://www.w3.org/2000/svg"
						width="24"
						height="24"
						fill="none"
						viewBox="0 0 24 24"
						aria-hidden="true"
						class="Icon-module--icon24--b74c8 SocialsNavigation-module--icon--fd20c ariaHidden"
						><g clip-path="url(#twitter-icon)"
							><path
								fill="currentColor"
								d="M21.187 5.348c1.078-.66 1.79-1.565 2.142-2.714-1.05.558-2.09.938-3.135 1.149-.938-1.008-2.124-1.519-3.549-1.519-1.392 0-2.564.488-3.525 1.44-.951.955-1.434 2.118-1.434 3.473 0 .412.051.778.154 1.092-4.1-.14-7.495-1.842-10.17-5.114-.455.801-.68 1.617-.68 2.46 0 1.735.731 3.108 2.194 4.121-.835-.07-1.57-.281-2.194-.628 0 1.237.365 2.273 1.092 3.183a4.828 4.828 0 0 0 2.822 1.72c-.417.108-.853.16-1.303.16-.417 0-.713-.033-.886-.113a4.722 4.722 0 0 0 1.72 2.456c.835.628 1.79.961 2.869.99-1.777 1.391-3.806 2.08-6.103 2.08-.563 0-.961-.009-1.205-.051C2.26 21 4.776 21.73 7.561 21.73c2.85 0 5.372-.722 7.565-2.165 2.194-1.44 3.811-3.211 4.852-5.297 1.045-2.086 1.56-4.257 1.56-6.52v-.624c1.013-.76 1.829-1.617 2.452-2.56a10.7 10.7 0 0 1-2.803.783z"
							/></g
						><defs
							><clipPath id="twitter-icon"><path fill="#fff" d="M0 0h24v24H0z" /></clipPath></defs
						></svg
					>
				</div>
			</figcaption>
		</figure>
	</div>
	{#if guests.length > 0}
		<h5>Featuring:</h5>
		<div class="featuring">
			{#each guests as guest}
				<figure>
					<img src={`https://github.com/${guest.github}.png`} alt="" />
					<figcaption>
						<p><a href={'/guests/' + guest.slug}>{guest.name}</a></p>
					</figcaption>
				</figure>
			{/each}
		</div>
	{/if}
</div>

<style>
	.guests-and-hosts {
		margin: 2rem 0;
		grid-column: start/end;
	}

	figure {
		display: grid;
		gap: 20px;
		grid-template-columns: 70px 1fr;
		margin: 0;
	}

	figure img {
		width: 100%;
		border-radius: 4px;
	}

	h5 {
		font-weight: 600;
		font-style: italic;
		margin: 2rem 0;
	}

	.featuring {
		display: flex;
		gap: 20%;
		margin-bottom: 2rem;
	}

	figcaption p {
		margin: 0;
	}
</style>
