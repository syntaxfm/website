 ...
 * Pull out the human issue number and a clean title for display in the archive.
 */
function parseSubject(subject: string): { issue_number: number | null; title: string } {
...
export const get_newsletter_archive = query(async () => {
	...
	const issues: NewsletterIssue[] = env.CONVERT_KIT_SECRET
		? await fetchBroadcastList()
		: [
				{
					published_at: new Date().toUTCString(),
					issue_number: 1337,
					title: 'ConvertKit API key not set (this is a fake issue)',
					id: 1337
				}
			];
	const count =
